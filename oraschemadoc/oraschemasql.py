...
# Copyright (C) Petr Vanek, <petr@scribus.info>, 2007
#
...
OracleCatalog = {
    ...
    'tab_partitions': OracleCatalogStatement(
                ...
                ownerSql='''select table_owner||'.'||table_name, partition_name, 
                            tablespace_name, high_value,
                            partition_position
                            from dba_tab_partitions %s order by table_owner||'.'||table_name, partition_position''',
    ...
    'views' : OracleCatalogStatement(
                ...
                ownerSql="""select owner||'.'||view_name, text from dba_views %s"""
                ),

    'mviews' : OracleCatalogStatement(
                userSql="""select mview_name, container_name, query, updatable from user_mviews""",
                ownerSql="""select owner||'.'||mview_name, container_name, query, updatable from dba_mviews %s"""
                ),
